...
const extractApiKey = (req, _res, next) => {
...
function readiness() {
    ...
    const overall = Math.round(dsa * 0.2 +
        core * 0.2 +
        projectsScore * 0.15 +
        communication * 0.15 +
        resumeScore * 0.1 +
    ...
    return {
        ...
        biggestBottleneck: [...all].sort((a, b) => a[1] - b[1])[0][0],
...
function fit(companyId) {
    ...
    const r = readiness();
    const score = Math.min(100, Math.round(skill +
        13 +
...
function tree() {
    return skills.map((s) => {
        ...
        if (s.id === 'graphs')
            s.status = unlocked ? 'SKILL_GAP' : 'LOCKED';
        return {
            ...s,
            prerequisites: pre,
...
function generateMission(wellbeing = 'FOCUSED') {
    const focus = skills.filter((s) => s.status !== 'MASTERED' && s.status !== 'LOCKED').sort((a, b) => a.proficiency - b.proficiency)[0] ||
        byId('trees');
...
// Auth Endpoints
app.post('/api/auth/register', validate(z.object({ name: z.string().min(2), email: z.string().email(), password: z.string().min(8) })), asyncHandler(async (req, res) => {
...
app.post('/api/auth/login', validate(z.object({ email: z.string().email(), password: z.string() })), asyncHandler(async (req, res) => {
...
app.put('/api/profile', auth, validate(z.object({ name: z.string().min(2).optional(), targetRole: z.string().optional(), availableHoursPerDay: z.number().int().min(1).max(12).optional() })), (req, res) => {
    Object.assign(user, req.body);
    ok(res, user);
});
app.get('/api/profile/skills', auth, (_, res) => ok(res, skills));
app.post('/api/profile/skills', auth, validate(z.object({ skillId: z.string(), proficiency: z.number().min(0).max(100) })), (req, res) => {
...
app.post('/api/profile/projects', auth, validate(z.object({ title: z.string(), description: z.string(), technologies: z.string() })), (req, res) => {
    const p = { id: 'project-' + Date.now(), ...req.body };
    projects.push(p);
    ok(res, p, 201);
});
app.get('/api/profile/connections', auth, (_, res) => ok(res, connections));
app.post('/api/profile/connections', auth, validate(z.object({ platform: z.string(), username: z.string() })), (req, res) => {
...
app.post('/api/learning/problem/:id/complete', auth, (req, res) => {
    ...
    if (!p)
        return fail(res, 'Problem not found', 404);
...
app.post('/api/learning/assessment/:id/submit', auth, validate(z.object({ score: z.number().min(0).max(100) })), (req, res) => {
...
app.post('/api/target-company', auth, validate(z.object({ companyId: z.string(), jobRoleId: z.string().optional() })), (req, res) => {
    if (!companies.some((c) => c.id === req.body.companyId))
        return fail(res, 'Company not found', 404);
...
app.post('/api/missions/wellbeing', auth, validate(z.object({ wellbeing: z.enum(['FOCUSED', 'OKAY', 'STRESSED']) })), (req, res) => ok(res, {
    wellbeing: req.body.wellbeing,
    mission: generateMission(req.body.wellbeing),
    message: req.body.wellbeing === 'STRESSED'
...
app.post('/api/missions/tasks/:id/complete', auth, (req, res) => {
    ...
    const task = m.tasks.find((t) => t.id === String(req.params.id));
    if (!task)
        return fail(res, 'Mission task not found', 404);
...
// AI Mentor Chat
app.post('/api/ai/chat', validate(z.object({
...
app.post('/api/ai/mock/question', validate(z.object({
    type: z.string(),
...
app.post('/api/ai/mock/evaluate', validate(z.object({
    type: z.string(),
    question: z.string(),
...
app.post('/api/interviews', auth, validate(z.object({ type: z.string(), companyId: z.string().optional(), question: z.string(), answer: z.string().min(5) })), asyncHandler(async (req, res) => {
...
app.get('/api/interviews/history', auth, (_, res) => ok(res, {
    attempts: interviews,
    trend: interviews
        .slice(0, 5)
        .reverse()
...
// Resume Endpoints
app.post('/api/resumes', auth, validate(z.object({ title: z.string(), content: z.string().min(20), targetRole: z.string().default('Software Engineer') })), (req, res) => {
...
app.post('/api/resumes/:id/analyze', auth, asyncHandler(async (req, res) => {
    const r = resumes.find((x) => x.id === String(req.params.id));
    if (!r)
        return fail(res, 'Resume not found', 404);
...
app.post('/api/resumes/:id/tailor', auth, validate(z.object({ bullet: z.string().optional(), action: z.enum(['improve', 'concise', 'keywords', 'quantify', 'company']).default('improve'), companyName: z.string().optional() })), asyncHandler(async (req, res) => {
    const r = resumes.find((x) => x.id === String(req.params.id));
    if (!r)
        return fail(res, 'Resume not found', 404);
...
app.post('/api/ai/resume/tailor', validate(z.object({
...
app.get('/api/career/future-scope', auth, (_, res) => ok(res, {
    targetRole: user.targetRole,
    futureDemand: 'HIGH',
    careerGrowth: 'STRONG',
    aiImpact: 'MODERATE',
...
app.get('/api/career/alternative-paths', auth, (_, res) => ok(res, [
