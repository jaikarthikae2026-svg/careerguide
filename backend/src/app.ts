...
// -------------------------------------------------------------
// 1. DATA MODELS & IN-MEMORY PERSISTENT STATE
// -------------------------------------------------------------
...
export const roleDiagnostics: Record<
  string,
  { id: string; skill: string; question: string; options: string[]; correctIndex: number; explanation: string }[]
> = {
...
// Learning Hub Lessons Collection
...
let learningLessons: LessonRecord[] = [
  {
    id: 'lesson-fe-1',
...
let projectSubmissions: ProjectEvidenceRecord[] = [
  {
    id: 'proj-sub-1',
    ...
    problemStatement:
      'Colleges and placement cells struggle with fragmented placement data, lacking real-time visibility into student skill readiness and department placement metrics.',
    ...
    decisionsNotes:
      'Implemented responsive CSS grid cards, memoized chart data transformations with useMemo, and added 12 automated unit tests with Vitest achieving 88% branch coverage.',
    status: 'Verified',
    score: 86,
    review: {
...
// Job Applications Collection (10 Stages)
...
let jobApplications: JobApplicationRecord[] = [
  {
    id: 'app-1',
    studentId: 'usr-student-1',
...
let podTasksState: Record<string, boolean> = {
  'task-pod-1': true,
  'task-pod-2': false,
  'task-pod-3': false,
};
...
let workReadyAttempts: { id: string; scenarioId: string; selectedOption: string; timestamp: string }[] = [];
...
let inAppNotifications: NotificationRecord[] = [
  {
    id: 'notif-1',
...
// Secure File Storage Metadata Collection
...
let secureFiles: FileMetadataRecord[] = [
  {
    id: 'file-1',
...
const token = (id: string, role: UserRole = 'student', email = 'divya@careeros.demo') =>
  jwt.sign({ sub: id, role, email }, secret, { expiresIn: '7d' });
...
const auth = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  try {
    const payload = jwt.verify(raw, secret) as { sub: string; role?: UserRole; email?: string };
...
app.put('/api/profile', auth, (req, res) => {
  ...
  if (req.body.location !== undefined) currentUser.location = req.body.location;
  if (req.body.email) currentUser.email = req.body.email;
  if (req.body.avatarUrl !== undefined) currentUser.avatarUrl = req.body.avatarUrl;

  if (currentUser.studentProfile) {
    if (req.body.college !== undefined) currentUser.studentProfile.college = req.body.college;
    if (req.body.degree !== undefined) currentUser.studentProfile.degree = req.body.degree;
    if (req.body.graduationYear !== undefined) currentUser.studentProfile.graduationYear = Number(req.body.graduationYear);
    if (req.body.targetRole !== undefined) currentUser.studentProfile.targetRole = req.body.targetRole;
    if (req.body.targetIndustry !== undefined) currentUser.studentProfile.targetIndustry = req.body.targetIndustry;
    if (req.body.preferredLanguage !== undefined) (currentUser.studentProfile as any).preferredLanguage = req.body.preferredLanguage;
    if (req.body.weeklyAvailabilityHours !== undefined) currentUser.studentProfile.weeklyAvailabilityHours = Number(req.body.weeklyAvailabilityHours);
    if (req.body.workModePreference !== undefined) currentUser.studentProfile.workModePreference = req.body.workModePreference;
    if (req.body.githubUrl !== undefined) (currentUser.studentProfile as any).githubUrl = req.body.githubUrl;
    if (req.body.portfolioUrl !== undefined) (currentUser.studentProfile as any).portfolioUrl = req.body.portfolioUrl;
    if (req.body.linkedinUrl !== undefined) (currentUser.studentProfile as any).linkedinUrl = req.body.linkedinUrl;
    if (req.body.bio !== undefined) (currentUser.studentProfile as any).bio = req.body.bio;
  }
...
app.post('/api/auth/switch-role', auth, (req, res) => {
  ...
  let name = 'Divya';
  if (role === 'mentor') name = 'Sneha Roy';
  if (role === 'recruiter') name = 'Vikram Seth';
  if (role === 'platform_admin') name = 'Platform Administrator';
...
app.get('/api/assessments/diagnostic', auth, (req, res) => {
  const roleQuery = (req.query.role as string) || currentUser.studentProfile?.targetRole || 'Junior Frontend Developer';
...
app.post('/api/projects/submit', auth, (req, res) => {
  const newProject: ProjectEvidenceRecord = {
...
app.post('/api/mentor/reviews/:id/respond', auth, (req, res) => {
  ...
  if (!proj) return fail(res, 'Review request not found', 404);
...
app.post('/api/mentor/reviews/:id/evaluate', auth, (req, res) => {
  const proj = projectSubmissions.find((p) => p.id === String(req.params.id));
  if (!proj) return fail(res, 'Project submission not found', 404);
...
app.post('/api/projects/:id/resubmit', auth, (req, res) => {
  ...
  if (req.body.liveDemoUrl) proj.liveDemoUrl = req.body.liveDemoUrl;
...
app.put('/api/applications/:id/stage', auth, (req, res) => {
  ...
  if (req.body.interviewNotes) appRecord.interviewNotes = req.body.interviewNotes;
  if (req.body.employerFeedback) appRecord.employerFeedback = req.body.employerFeedback;
  if (req.body.rejectionReason) appRecord.rejectionReason = req.body.rejectionReason;
...
// -------------------------------------------------------------
// 11. SKILL MISSIONS ENGINE
// -------------------------------------------------------------

export interface SkillMissionDefinition {
  skillName: string;
  missionTitle: string;
  whyItMatters: string;
...
const skillMissionsStore: Record<string, SkillMissionDefinition> = {
  React: {
    skillName: 'React',
