...
function createGenAIClient(customApiKey?: string): GoogleGenAI | null {
...
export const aiService = {
  getAiStatus(customApiKey?: string) {
    const key = getApiKey(customApiKey);
    return {
  ...
  async chatWithCareerMentor(
    messages: ChatMessage[],
    studentContext: StudentContext = {},
    customApiKey?: string,
  ): Promise<{ message: string; provider: 'gemini' | 'mock' }> {
...
Evaluate the candidate objectively and respond strictly in valid JSON format matching this schema:
{
  ...
  async generateInterviewQuestion(
    ...
    customApiKey?: string,
  ): Promise<{ question: string; topic: string; provider: 'gemini' | 'mock' }> {
    const ai = createGenAIClient(customApiKey);
...
function fallbackMentorResponse(prompt: string, context: StudentContext): string {
...
function fallbackInterviewEvaluation(question: string, answer: string): InterviewEvaluationResult {
...
function fallbackQuestion(type: string, topic?: string, company?: string): string {
...
function fallbackResumeAnalysis(content: string): ResumeAnalysisResult {
...
function fallbackResumeTailor(bullet: string, action: string, company?: string): ResumeTailorResult {
